...
iso_mapping_un3 = df_current_3.iloc[:, :2].drop_duplicates()
#print(iso_mapping.head())
iso_mapping_un3.to_csv("/Users/Danjing 1/Lingsu/Jobs/2024 WB STC/Sector/Process/iso_mapping_un3.csv", index=False)
...
# Apply the function with specific column names and the new column name 'missing_AB','missing_GH', 'missing_JK'
...
def calculate_missing_combination3(df, col1, col2, col3, col_sum, new_col_name):
    ...
    return df
# Apply the function with specific column names and the new column name 'missing_MNO'
pivot_df_current_3 = calculate_missing_combination3(pivot_df_current_3, 'M', 'N', 'O', 'M+N+O', 'missing_MNO')

## Then check all the other sectors: C, D, E, F, I, L. Please note sector P can be missing, so not included.
...
# If footnotes are related to tax ans subsidy add or minus for all other sectors, then we can still use 0.005.
...
### Save a copy of current discard column, only considering the scores, without considering the number of sectors.
pivot_df_current_3['discard_on_score'] = pivot_df_current_3['discard']
...
pivot_df_current_3['discard_on_sector'] = conditions
...
df_current_3_int_discard.reset_index(inplace=True, drop=True)
print(df_current_3_int_discard.tail(10))
...
print(df_current_3_int.tail(10))

# Keep only columns interested.
columns_to_keep = ['country', 'iso3', 'year','series_code','Footnotes','part','sector_missing','score','discard_on_score', 'discard_on_sector']
df_current_3_int = df_current_3_int.loc[:, columns_to_keep]
df_current_3_int.reset_index(inplace=True, drop=True)
print(df_current_3_int.tail(10))
...
df_current_3_int = df_current_3_int.iloc[:,0:4]
print(df_current_3_int.head())
df_current_3_int.to_csv("/Users/Danjing 1/Lingsu/Jobs/2024 WB STC/Sector/Process/un3_current_part1.csv")
#
# # ### issues:
# # # ###1) GDP footntoes
...
## bi*g footnote check with Yann:
# Nauru, series 100, 2004-2006, sector E (Electricity, gas and water supply) only, has footnote 46 (part2).
# Part 2 footnote 46: The figure is negative because value added data are provided at producer prices, including
# subsidies that were necessary to restructure the water and electricity industries and make them independent from
# the Nauru Phosphate Corporation (NPC).
